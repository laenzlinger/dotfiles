...
- [ ] Fix `Shift+Print` screenshot — unquoted `$(...)` in sway exec, may break on spaces in path
...
- [ ] Remove or document commented `exec sway` in `.zshrc`
- [ ] Remove or document commented Obsidian autostart in `sway/config`
...
- [ ] Verify `base16.config` generation — sway includes it but no managed source
- [ ] Make `swaync/generate-colors.sh` run automatically via tinty hook
- [x] Hardcoded username `laenzi` in `.zshrc` pipx PATH — use `$HOME` instead
- [ ] `colors.css` for waybar is static (Darktooth) — should be auto-generated by tinty
